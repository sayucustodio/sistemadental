(function($) {
	...
	//CLOSE PICKER
	.on('click',function(e) {
		if(picker && !picker.input.data('inline')) {
	...
	$.fn.dateDropper = function(options) {
		return $(this).each(function(){
			if($(this).is('input')&&!$(this).hasClass('picker-input')) {
				...
				var
					...
					picker_inline = input.data('inline'),
				...
				if (picker_inline) {
					...
					$($('#'+$(this).data('id')).detach()).insertAfter($(this));
					$('#'+$(this).data('id')).css({
						'position':'initial',
						'z-index':'0',
						'-webkit-transform':'translateX(0)',
						'-moz-transform':'translateX(0)',
						'-ms-user-select':'translateX(0)',
						'-o-user-select':'translateX(0)',
		...
		.focus(function(e){
			...
			var picker_inline = $(this).data('inline');
			
			if (picker_inline==false) {
